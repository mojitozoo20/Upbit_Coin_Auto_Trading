        ...
        while True:
            try:
                ...
                if i == (5 * 10):
                    ...
                    print(f"{TICKER} 보유량:{upbit.get_balance_t(self.ticker)}, 보유KRW: {cash},  hold_flag= {hold_flag}, wait_flag= {wait_flag}, signal = {curr.SAR < curr.BBAND_LOWER and curr.senkou_spna_A < curr.BBAND_MIDDLE and curr.senkou_spna_B < curr.BBAND_MIDDLE and curr.close < curr.BBAND_UPPER and curr.senkou_spna_A > curr.senkou_spna_B}")
...
while True:
    now = datetime.datetime.now()
    if now.microsecond < 10000:  # 대기 후 매분 정각에 시작
        q = queue.Queue()
        Producer(q).start()
        Consumer(q).start()
        break
